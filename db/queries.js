...
async function signupUser(db, userData) {
  //first find the user to see if he exists
  const existingUser = await db.collection('users').findOne({ email: userData.email });
  if (existingUser) {
    throw new Error('A user with that email already exists.');
  }

  //if not exist then insert the user
...
async function deleteTask(db, taskId) {
  const task = await db.collection('tasks').deleteOne({ _id: taskId });
  return { deletedCount: task.deletedCount };
}
...
async function searchNotes(db, ownerId, tags, projectId) {
  //in se mtlb in tags arr me se koi bhi match krta ho
  const filter = { ownerId, tags: { $in: tags } }
  //agr proj id provided ha to add it to filter
  if (projectId){
